...
Ports for background steps currently do not convert when downgrading to v0 yaml. This applies to all providers that expose container ports for services.
...
When downgrading to v0 yaml, `ports` is lost:
...
Manually set the correct `portBindings` values, for example:
