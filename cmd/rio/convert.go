...
func (c *convertCommand) run(ctx *kingpin.ParseContext) error {
	...
	// format to the harness v0 yaml format.
	converter := rio.New(
	...
	// - Downgrade is not supported since rio converter only supports
	//   converting to v0 yaml format
...
func registerConvert(app *kingpin.CmdClause) {
	...
	cmd := app.Command("convert", "convert a rio yaml").
		Action(c.run)

	cmd.Arg("input", "path to rio yaml directory or file").
		Default(".rio/pipeline.yml").
		StringVar(&c.input)
